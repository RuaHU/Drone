...
class Anafi():
    def __init__(self, drone_type="Anafi", ip_address=None):
        ...
        self.pcmd_buffer = []
        _thread.start_new_thread(self._pcmd, ('_pcmd',))
    ...
    def _pcmd(self, name):
        while True:
            y, f, r, v, d = 0, 0, 0, 0, 0
            if len(self.pcmd_buffer):
                while len(self.pcmd_buffer) > 0:
                    func, args = self.pcmd_buffer.pop(0)
                    func(*args)
                    print('pcmd:', args)
                    continue
                '''
                    if func == self.drone_connection.send_pcmd_command:
                        cmd_t, _y, _f, _r, _v, _d = args
                        md = max(d, _d)
                        y = (_y * _d + y * d) / md
                        f = (_f * _d + f * d) / md
                        r = (_r * _d + r * d) / md
                ...
                r = np.clip(r, -100, 100)
                v = np.clip(v, -100, 100)
                print('sending message:', y, f, r, v, d)
                self.drone_connection.send_pcmd_command(cmd_t, y, f, r, v, d)'''
            else:
                time.sleep(0.01)


    def fly_direct_xxx(self, yaw, forward, rotation, vertical, duration):
        '''
        yaw:        [-100->100]% of the max yaw speed
        forward:    [-100->100]% of the max
        '''
        yaw = np.clip(yaw, -100, 100)
        forward = np.clip(forward, -100, 100)
        rotation = np.clip(rotation, -100, 100)
        vertical = np.clip(vertical, -100, 100)

        command_tuple = self.command_parser.get_command_tuple("ardrone3", "Piloting", "PCMD")


        #self.drone_connection.send_pcmd_command(command_tuple, yaw, forward, rotation, vertical, duration)
        self.pcmd_buffer.append([self.drone_connection.send_pcmd_command,
                                [command_tuple, yaw, forward, rotation, vertical, duration]])
    ...
    def set_gimbal_target(self, gimbal_id, yaw, pitch, roll):
        """
        set camera gimbal
        ...
        return self.drone_connection.send_param_command_packet(cmd_tuple, param_tuple, param_type_tuple, ack=True)
    ...
    def pan_tilt_camera(self, tilt_degrees, pan_degrees):
        ...
        if(self.drone_type == "Anafi"):
            ...
            self.sensors.CameraMoveEnded_pan = False
            self.sensors.CameraMoveEnded_tilt = False
            ...
            while (not self.sensors.CameraMoveEnded_pan) and (not self.sensors.CameraMoveEnded_tilt):
                self.smart_sleep(0.01)
        else:
    ...
    def set_max_tilt_rotation_speed(self, speed):
        ...
        self.sensors.max_pitch_roll_rotation_speed_changed = False
    ...
    def set_max_vertical_speed(self, speed):
        ...
        self.sensors.max_vertical_speed_changed = False
        command_tuple = self.command_parser.get_command_tuple("ardrone3", "SpeedSettings", "MaxVerticalSpeed")
        self.drone_connection.send_param_command_packet(command_tuple, param_tuple=[speed], param_type_tuple=['float'], sensors = self.sensors)
    ...
    def set_max_rotation_speed(self, speed):
        ...
        self.sensors.max_rotation_speed_changed = False
