...
class Tello:
    ...
    def __init__(self,
        ...
        #image reading
        self._image_queue = queue.Queue(2)
        self.decoder = h264decoder.H264Decoder()
        self.socket_video = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)  # socket for receiving video stream
        self.socket_video.bind(('', self.VS_UDP_PORT))
        _thread.start_new_thread(self._receive_video_thread, ('tello video receiving',))


    def read(self, timestamp = False):
        '''get image in block mode'''
        if timestamp:
            #return image and timestamp
            return self._image_queue.get()
        else:
            #return image only
            return self._image_queue.get()[0]

    def _receive_video_thread(self, name):
        packet_data = b''
        while True:
            try:
    ...
    def _h264_decode(self, packet_data):
        ...
        for framedata in frames:
            ...
            if frame is not None:
                ...
                frame = frame[:, :w, ::-1]#RGB2BGR opencv format
                res_frame_list.append([frame, timestamp])
    ...
    def get_state_field(self, key: str):
        ...
        if key in state:
            ...
            print('Could not get state property:', key, state)
    ...
    def send_command_with_return(self, command: str, timeout: int = RESPONSE_TIMEOUT) -> str:
        ...
        while not responses:
            if time.time() - timestamp > timeout:
                ...
                return message
            time.sleep(0.02)  # Sleep during send command
    ...
    def raise_result_error(self, command: str, response: str) -> bool:
        ...
        self.LOGGER.error("Command '{}' was unsuccessful for {} tries. Latest response:\t'{}'"
    ...
    def rotate_clockwise(self, x: int):
        ...
        Arguments:
            x: 1-360
        """
        self.send_command_without_return("cw {}".format(x))
    ...
    def rotate_counter_clockwise(self, x: int):
        ...
        Arguments:
            x: 1-3600
        """
        self.send_command_without_return("ccw {}".format(x))
