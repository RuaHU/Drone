...
class DataAugmentation():
    ...
    def mold_box(self,):
        if hasattr(self, 'image'):
            height, width, _ = self.image[0].shape
        else:
        ...
        for boxes in self.bbox:
            ...
            boxes[:, 0][boxes[:, 0] > width] = width
            boxes[:, 2][boxes[:, 2] > width] = width
            boxes[:, 1][boxes[:, 1] > height] = height
    ...
    def unmold(self, bboxes, meta):
        ih, iw = meta[0][4:6]
        window = meta[0][7:11]
        if self.config.M in ['dla_34', 'dla_34_ext', 'dla_34_new']:
            ratio = meta[0][11]
            dh, dw = window[:2]
    ...
    def mold(self,):
        ...
        for i in range(1):
            ...
            elif self.config.M in ['dla_34', 'dla_34_ext', 'dla_34_new']:
